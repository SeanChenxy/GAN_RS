...
class Visualizer():
    ...
    def plot_current_errors(self, epoch, counter_ratio, opt, errors):
        if not hasattr(self, 'plot_data_G'):
            self.plot_data_G = {'X':[],'Y':[], 'legend':list(errors.keys())[:int(len(errors)/2)]}
        if not hasattr(self, 'plot_data_D'):
            self.plot_data_D = {'X': [], 'Y': [], 'legend': list(errors.keys())[int(len(errors)/2):]}
