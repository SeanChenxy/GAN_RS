...
class UnetGenerator(torch.nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
                 norm_layer=torch.nn.BatchNorm2d, use_dropout=False, gpu_ids=[]):
    ...
    def forward(self, input):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor):
            return torch.nn.parallel.data_parallel(self.model, input, self.gpu_ids)
        else:
...
# netG = UnetGenerator(3, 3, 7, 64, norm_layer=functools.partial(torch.nn.BatchNorm2d, affine=True),
#                      use_dropout=True, gpu_ids=[])
netG = ResnetGenerator(3, 3, 64, norm_layer=functools.partial(torch.nn.BatchNorm2d, affine=True), use_dropout=True, n_blocks=9,
                       gpu_ids=[])
