...
keys = ['real_A','gw_A', 'pb_A', 'clahe_A', 'DM_A', 'CM_A', 'UIR_A', 'pix_B', 'real_B', 'fake_B']
...
pro = ['Origin', 'GW', 'PB', 'CLAHE', 'DM', 'CM', 'RBLA', 'pix2pix', 'FRS', 'GAN-RS']
# label_bcd = ['(b)', '(c)', '(d)']
...
for i, key in enumerate(keys):
    # print(key,dets )
    # key = name[0].split('_')[-2]+'_'+name[0].split('_')[-1]
    # key = key[:-4]
    type = 'jpg' if key in ['UIR_A', 'gw_A', 'CM_A', 'DM_A', 'clahe_A', 'pb_A'] else 'png'

    name_ssd = ssd_path+'Video3_97_%s.%s' % (key, type)
    # img4 = cv2.resize(cv2.cvtColor(cv2.imread(name_ssd,1), cv2.COLOR_BGR2RGB), (int(img3.shape[1]/2), img3.shape[0]))   # queryImage
    img4 = cv2.cvtColor(cv2.imread(name_ssd,1), cv2.COLOR_BGR2RGB)   # queryImage

    # print(img4.shape, img3.shape)
    # img_show = np.concatenate((img3, img4),1)
    ...
    for object in det_res[key]:
        ...
        ax.add_patch(rect)
    plt.title(pro[i], fontdict=font_label)
plt.show()
